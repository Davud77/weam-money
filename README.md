...
// Берём токен из localStorage, чтобы авторизовать запросы
function getAuthHeaders(): Record<string, string> {
  const token = localStorage.getItem('token');
  return token ? { Authorization: `Bearer ${token}` } : {};
}

// Получаем последний день текущего месяца в формате "yyyy-mm-dd"
...
// Структура для «динамики по датам»
type DayEntry = {
  date: string;
  incomePlan: number;
  expensePlan: number;
...
// Набор полей числовых, чтобы добавить в dayMap
...
// Конфигурация линий (с цветами и ширинами)
const linesConfig = [
  {
    key: 'incomePlan',
    color: '#5d9cec',
...
const DashboardPage: React.FC<DashboardPageProps> = ({ showError }) => {
  ...
  // Цвета для итоговой таблицы (План/Факт)
  ...
  // Подсчет общей суммы дохода/расхода
  ...
  // Делим на план/факт, причём планам даём дату = последний день месяца
  ...
  // Подсчёт доход/расход/прибыль + рентабельность (план/факт/итого)
  ...
  // Готовим данные для круговых диаграмм
  const pieDataPlan = [
  ...
  // Готовим BarChart (оборот по контрагентам)
  const contractorMap: { [key: string]: number } = {};
  filteredTransactions.forEach((tx) => {
    const key = tx.contractor || 'Не указан';
    contractorMap[key] = (contractorMap[key] || 0) + tx.total;
  });
  const barData = Object.entries(contractorMap).map(([contractor, sum]) => ({
    contractor,
    total: sum,
  }));
  ...
  // Заполняем dayMap, складываем доход/расход в нужные поля
  ...
  filteredTransactions.forEach((tx) => {
    ...
    const isPlan = !tx.date;
    if (isPlan) {
      if (tx.operationType === 'Доход') {
        addToDayEntry(realDate, 'incomePlan', tx.total);
      } else {
        addToDayEntry(realDate, 'expensePlan', tx.total);
      }
    } else {
      // факт
      if (tx.operationType === 'Доход') {
        addToDayEntry(realDate, 'incomeFact', tx.total);
      } else {
        addToDayEntry(realDate, 'expenseFact', tx.total);
      }
    }
  });
  ...
  const [mode, setMode] = useState<'daily' | 'cumulative'>('cumulative'); // по умолчанию "накопительный"
  const handleModeChange = (event: SelectChangeEvent<string>) => {
    setMode(event.target.value as 'daily' | 'cumulative');
  };

  // Если "cumulative" — накапливаем по датам, иначе просто rawData
...
{/* ТРИ КРУГОВЫЕ ДИАГРАММЫ (План, Факт, Итого) + ТАБЛИЦА */}
      ...
      {/* BAR CHART: Суммарный оборот по контрагентам */}
